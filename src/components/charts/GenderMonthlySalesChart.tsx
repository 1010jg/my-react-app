...
export default function GenderMonthlySalesChart() {
  ...
  useEffect(() => {
    fetch('/data/cleaned_output.json')
      .then(res => res.json())
      .then((data: RecordRow[]) => {
        // auto-detect months present and sort
        const monthsSet = new Set<string>()
        data.forEach(r => { if (r.year_month) monthsSet.add(r.year_month) })
        const months = Array.from(monthsSet).sort()

        // ensure months sorted chronologically; if none, default to 2025 months
        ...
        const sumBy = (gender: string) => labels.map(m => 
          data
            .filter(d => (d.year_month === m) && (String(d.gender).trim() === gender))
        ...
        setChartData({
          ...
          datasets: [
            ...
            // include unknown/other if present
